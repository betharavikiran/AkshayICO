...
const targetApi = 'http://localhost:9545';
...
const akshayIco = web3.eth.contract(abi).at(contractAddress);

/// read and display values
writeValue('contractAddress', contractAddress);
writeValue('totalRaised', toEthString(akshayIco.totalRaised()));

const startDate = hexSecondsToMillis(akshayIco.startTimestamp());
const endDate = startDate + hexSecondsToMillis(akshayIco.durationSeconds());
writeValue('startDate', new Date(startDate).toLocaleString());
writeValue('endDate', new Date(endDate).toLocaleString());

writeValue('minCap', toEthString(akshayIco.minCap()));
writeValue('maxCap', toEthString(akshayIco.maxCap()));
